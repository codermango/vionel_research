...
function followmouse(e) {
	...
	if (typeof e != "undefined") {
		...
		if (docheight - e.pageY < defaultimageheight + 2 * offsetfrommouse[1]) {
			...
			ycoord += e.pageY - Math.max(0, (2 * offsetfrommouse[1] + defaultimageheight + e.pageY - docheight - document.scrollTop));
		} else {
			ycoord += e.pageY;
		}
	...
	if (document.getElementById("moviefingerprint2").offsetTop > e.pageY) {
		var rratio = Math.round(document.getElementById("big1").naturalWidth * 20 / document.getElementById("big1").width);
		dir = moviePoster.src.replace(".jpg", "/")
	} 
